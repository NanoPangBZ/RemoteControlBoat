...
#define Usart_Rx_SbufferSize  32
#define Usart_Tx_SbufferSize  32

//每行第一个元素表示该缓存区存放的数据个数
static uint8_t USART_Rx_Sbuffer[2][Usart_Rx_SbufferSize + 1] = {{0},{0}};
static uint8_t USART_Tx_Sbuffer[2][Usart_Tx_SbufferSize + 1] = {{0},{0}};
//获取缓存区数据长度
...
void Usart_Tx_Flag_Clear(uint8_t USARTx);                 //清除DMA软件在忙标志位
