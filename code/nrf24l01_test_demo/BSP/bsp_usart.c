...
void USART_DMA_Config(void)
{
    ...
    for(uint8_t temp=0;temp<2;temp++)
    {
        ...
        DMA_Init(Usart_TargetDMA_Channel[temp],&DMA_InitStruct);
    ...
    for(uint8_t temp=0;temp<2;temp++)
        DMA_ITConfig(Usart_TargetDMA_Channel[temp],DMA_IT_TC,ENABLE);
}
...
uint8_t Usart_Send(uint8_t USARTx,uint8_t *dat,uint8_t len)
{
    if(Usart_BusyCheck(USARTx)==0)
    {
        ...
        Usart_TargetDMA_Channel[USARTx-1]->CNDTR = len;
        Usart_TargetDMA_Channel[USARTx-1]->CCR |= DMA_CCR1_EN;
        return 0;
    }
    return 1;
}
...
uint8_t Usart_SendString(uint8_t USARTx,uint8_t*str)
{
    if(Usart_BusyCheck(USARTx) == 0)
    {
        while(*str!='\0' && Tx_Len(USARTx) < Usart_Tx_SbufferSize)
        {
            USART_Tx_Sbuffer[USARTx-1][Tx_Len(USARTx)+1] = *str;
            USART_Tx_Sbuffer[USARTx-1][0]++;
            str++;
        }
        Usart_TargetDMA_Channel[USARTx-1]->CNDTR = Tx_Len(USARTx);
        Usart_TargetDMA_Channel[USARTx-1]->CCR |= DMA_CCR1_EN;
        return 0;
    }
    return 1;
...
{
    //判断缓存区是否满载
    if(USART_Rx_Sbuffer[USARTx-1][0] < Usart_Rx_SbufferSize)
    {
        USART_Rx_Sbuffer[USARTx-1][ Rx_Len(USARTx) + 1 ] = dat;
        USART_Rx_Sbuffer[USARTx-1][0]++;
    }else
        USART_Rx_Sbuffer[USARTx-1][0] = Usart_Rx_SbufferSize + 1;
}

void Usart_Tx_Flag_Clear(uint8_t USARTx)
{
...
void DMA1_Channel4_IRQHandler(void)
{
    ...
    {
        Usart_Tx_Flag_Clear(1);
        Usart_TargetDMA_Channel[0]->CCR &= (uint16_t)(~DMA_CCR1_EN);
...
void DMA1_Channel7_IRQHandler(void)
{
    ...
    {
        Usart_Tx_Flag_Clear(2);
        Usart_TargetDMA_Channel[1]->CCR &= (uint16_t)(~DMA_CCR1_EN);
